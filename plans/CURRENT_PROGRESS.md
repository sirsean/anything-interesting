...
| M3 | **Watchlist fix in progress** | Shipped 2026-05-10: Gamma + CLOB, snapshots, Strategy A/B, etc. **2026-05-19:** Watchlist now seeds from Polymarket **Breaking** feeds (`GET https://polymarket.com/api/biggest-movers?category=politics|world` — same as `/breaking/politics` and `/breaking/world`), then backfills from Gamma volume only if under 50 kept. Root cause of empty prod watchlist: generic `/markets?order=volume24hr` returns sports-heavy rows with empty tags → Kimi rejected all. **Next:** deploy + `POST /ops/refresh-watchlist` smoke; confirm D1 `markets` populated. |
...
| — | **Market research (Brave → Kimi)** | **Implemented.** Always-on Brave News + page-fetch for Strategy B flagged moves; research URLs in `llm_reasoning_log` + cluster UI; no digest gate. Secret: `BRAVE_SEARCH_API_KEY`. Details: [`OPTIONS-search-on-miss.md`](OPTIONS-search-on-miss.md). **Next:** put secret + deploy; smoke a market-driven cluster for research sources. |
...
**2026-07-04 (Strategy B corruption fix):** Recent digests showed a Polymarket market paired with an unrelated headline (e.g. "France vs Paraguay" under a US–Iran diplomatic-meeting market) and Kimi summaries saying "articles don't match." Root cause was **Strategy B** (`src/snapshots.ts`), not headline clustering: (1) `keywordsFromTitle` kept date tokens (`july`, `2026`) and generic words (`president`, `meeting`), and `searchArticles` used `OR` LIKE — so one common word dragged in dozens of unrelated articles; (2) `upsertMarketDrivenCluster` then **reassigned** those articles' `cluster_id` to the market cluster (`UPDATE articles SET cluster_id`), corrupting real news clusters; (3) the digest/embed headlined the most-recent stolen article title instead of the market question. Fixes: expanded stopword/date filter; `searchArticles` requires ≥2 keyword overlap and ranks by overlap; new `filterRelevantArticles` (Kimi) gates hits; new `market_cluster_articles` reference table (`migrations/0008`) cites articles **without** stealing them; market-driven embeds/digest now headline `representative_title` (the market question). Chose fix-forward (old corrupted clusters age out). Deployed.

**2026-07-03 (cluster eval harness):** Offline replay harness: `npm run cluster:eval` → `POST /ops/cluster-eval/prepare` (embed once to KV) + `/ops/cluster-eval/run` per preset. Simulates configurable cosine thresholds; Kimi (not GLM) verifies cluster coherence in eval only. **Eval on 200 articles / 72h:** prod 8.2% multi-source → **tuned_b 11.0%** (cosineLow 0.74, cosineSame 0.80, `always_merge_gray`); Kimi 100% within-cluster coherence on tuned_b sample. **Prod clustering updated** to tuned_b thresholds.

**2026-07-03 (Polymarket event URLs):** Neg-risk / multi-outcome Gamma markets expose a **per-outcome market slug** (e.g. `will-alesa-mengesha-be-the-next-prime-minister-of-ethiopia`) but Polymarket `/event/{slug}` pages use the **parent event slug** (`next-prime-minister-of-ethiopia`). We now persist `event_slug`, `event_title`, `group_item_title` from Gamma (`migrations/0007_markets_event_slug.sql`), build links via `polymarketEventUrl()`, and show event title + outcome label in the Markets UI and cluster Polymarket cards. Very low Yes prices display as `<1%` instead of `0%`. **Deployed** `61f92c1`; migration `0007` applied local+remote; `POST /ops/refresh-watchlist` backfilled 24 markets.
...
_Last updated: 2026-08-07 — Strategy B always-on Brave News + page-fetch research; research URLs on cluster UI. Prior: 2026-07-12 cluster OG meta._

**2026-08-07 (market research):** Strategy B flagged moves always call Brave News (`pd` then `pw`) and page-fetch top URLs before Kimi explain. Research persisted in `llm_reasoning_log.research[]` and exposed as `cluster.research` in the API / cluster detail page. Explainer prompt no longer nudges “market positioning” filler. No digest gate. Operator: `wrangler secret put BRAVE_SEARCH_API_KEY`.
